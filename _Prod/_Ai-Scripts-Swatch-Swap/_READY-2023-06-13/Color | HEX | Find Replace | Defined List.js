...
// 1. Defines a list of source and target hex color mappings.
// 2. Gets the active document in Adobe Illustrator.
// 3. Calls the replaceHexColors() function to replace the specified hex colors in the document.
// 4. Iterates over each color mapping and retrieves the source and target hex colors.
// 5. Converts the hex colors to RGB colors.
// 6. Iterates over each path or item in the document.
// 7. Checks if the item has a fill color and if the fill color matches the source color.
// 8. If a match is found, replaces the item's fill color with the target color.
// 9. Checks if the item has a stroke color and if the stroke color matches the source color.
// 10. If a match is found, replaces the item's stroke color with the target color.
// 11. Displays an alert message indicating that the swatches have been swapped to "DM".
// The script essentially replaces specific hex colors with their corresponding target colors in the document and displays an alert to notify the user about the completion of the process.
