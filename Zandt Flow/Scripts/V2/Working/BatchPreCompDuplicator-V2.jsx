// BatchPreCompDuplicator-format-fixed.jsx
...
(function() {
    ...
    okButton.onClick = function() {
        ...
        processBatchDuplication(
            ...
            movRadio.value, // Pass the selected format (false for MP4, true for MOV)
            debugCheck.value
        );
    ...
    function processBatchDuplication(sourcePreComp, sourceFPOFootage, footageDir, fileFilter, namingOption, addToRenderQueue, useMOVFormat, debugMode) {
        ...
        if (addToRenderQueue && createdComps.length > 0) {
            try {
                ...
                for (var i = 0; i < createdComps.length; i++) {
                    ...
                    var fileExtension = useMOVFormat ? ".mov" : ".mp4";

                    if (useMOVFormat) {
                        // For MOV, use the Lossless template directly like in the MOV version
                        try {
                            outputModule.applyTemplate("Lossless");
                        } catch (e) {
                            if (debugMode) {
                                alert("Could not apply Lossless template: " + e.toString());
                            }
                        }
                    } else {
                        // For MP4, try to apply H.264 templates
                        try {
