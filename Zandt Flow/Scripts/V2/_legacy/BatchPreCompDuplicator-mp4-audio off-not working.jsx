        ...
        if (addToRenderQueue && createdComps.length > 0) {
            try {
                ...
                for (var i = 0; i < createdComps.length; i++) {
                    ...
                    // Disable audio - specifically set to "OFF" not "Auto"
                    try {
                        var omSettings = outputModule.getSettings();

                        // Debug: If in debug mode, show the current audio output setting
                        if (debugMode) {
                            alert("Current Audio Output setting: " + omSettings["Audio Output"]);
                        }

                        // Try different variations of "Off" value
                        var offValues = ["Off", "OFF", false, 0, "off"];
                        var audioSet = false;

                        for (var v = 0; v < offValues.length; v++) {
                            try {
                                omSettings["Audio Output"] = offValues[v];
                                outputModule.setSettings(omSettings);
                                audioSet = true;

                                // Verify the setting took effect
                                var verifySettings = outputModule.getSettings();
                                if (debugMode) {
                                    alert("Set Audio Output to " + offValues[v] +
                                        ". New value: " + verifySettings["Audio Output"]);
                                }

                                // If we successfully set it, break the loop
                                break;
                            } catch (e) {
                                if (debugMode) {
                                    alert("Failed to set Audio Output to " + offValues[v] + ": " + e.toString());
                                }
                            }
                        }

                        // If none of the above worked, try a different approach with format settings
                        if (!audioSet) {
                            try {
                                // Try to modify format options directly
                                var formatOptions = new Object();
                                formatOptions.audioEnabled = false;  // Disable audio in format options
                                formatOptions.audioChannels = 0;     // Set audio channels to 0
                                formatOptions.audioSampleRate = 0;   // Set sample rate to 0

                                // Apply these format options
                                outputModule.setSettings({Format: outputModule.getSettings().Format, "Format Options": formatOptions});

                                if (debugMode) {
                                    alert("Applied format options to disable audio");
                                }
                            } catch (e) {
                                if (debugMode) {
                                    alert("Failed to set format options: " + e.toString());
                                }
                            }
                        }
                    } catch (e) {
                        if (debugMode) {
                            alert("Error disabling audio: " + e.toString());
                        }
                    }
