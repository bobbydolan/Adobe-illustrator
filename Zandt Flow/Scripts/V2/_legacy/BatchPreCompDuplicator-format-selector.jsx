// BatchPreCompDuplicator-format-selector.jsx
...
// It includes an option to choose between MP4 and MOV output formats.
//
// Author: DevMate
// Created: 2023
// Modified: 2023 - Added format selection option
...
(function() {
    ...
    // Render format options (only visible when Add to Render Queue is checked)
    var renderFormatGroup = outputGroup.add("group");
    renderFormatGroup.orientation = "row";
    renderFormatGroup.alignChildren = ["left", "center"];
    renderFormatGroup.spacing = 10;
    renderFormatGroup.enabled = false; // Initially disabled

    renderFormatGroup.add("statictext", undefined, "Render Format:");
    var mp4Radio = renderFormatGroup.add("radiobutton", undefined, "MP4 (H.264)");
    var movRadio = renderFormatGroup.add("radiobutton", undefined, "MOV (QuickTime)");
    mp4Radio.value = true; // MP4 is default

    // Enable/disable render format options based on render queue checkbox
    renderQueueCheck.onClick = function() {
        renderFormatGroup.enabled = renderQueueCheck.value;
    };
    ...
    okButton.onClick = function() {
        ...
        processBatchDuplication(
            ...
            // mp4Radio.value, // Pass the selected format (true for MP4, false for MOV)
            !movRadio.value, // Pass the selected format (true for MP4, false for MOV)
    ...
    function processBatchDuplication(sourcePreComp, sourceFPOFootage, footageDir, fileFilter, namingOption, addToRenderQueue, useMP4Format, debugMode) {
        ...
        if (addToRenderQueue && createdComps.length > 0) {
            try {
                ...
                for (var i = 0; i < createdComps.length; i++) {
                    ...
                    try {
                        // Select templates based on chosen format
                        var templateNames = [];
                        var fileExtension = "";

                        if (useMP4Format) {
                            // MP4/H.264 templates
                            templateNames = ["H.264", "H264", "MP4", "H.264 - Match Source - High bitrate"];
                            fileExtension = ".mp4";
                        } else {
                            // MOV/QuickTime templates
                            templateNames = ["QuickTime", "ProRes", "Animation", "Apple ProRes 422", "Apple ProRes 422 HQ"];
                            fileExtension = ".mov";
                        }
                        ...
                        if (!templateApplied) {
                            ...
                            outputModule.setSettings({
                                "Output File Info": {
                                    ...
                                    "File Name": createdComps[i].name + fileExtension
                                }
                            });
                        }
                    ...
                    if (renderFolder) {
                        try {
                            var fileExtension = useMP4Format ? ".mp4" : ".mov";
                            var outputPath = renderFolder.fsName + "/" + createdComps[i].name + fileExtension;
                            outputModule.file = new File(outputPath);
                        } catch (e) {
                            if (debugMode) {
                                alert("Could not set output path: " + e
                                    .toString());
                                }
                            }
                        }
